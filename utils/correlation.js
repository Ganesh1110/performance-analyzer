    ...
    return [
      {
        ...
        recommendation:
          "Issue may be native-only (image loading, native module, etc.)",
      },
    ];
  }
  ...
  const scored = candidateCommits.flatMap((commit) => {
    return commit.components.map((comp) => {
      ...
      const costScore = Math.min(
        comp.duration / CONFIG.thresholds.renderTime.critical,
        1,
      );
...
function correlateMemoryWithComponents(memoryData, componentRenderMap) {
  ...
  componentRenderMap.forEach((renders, componentName) => {
    // Pre-index render timestamps into a Set of 100ms buckets
    const renderBuckets = new Set(
      renders.map((r) => Math.floor(r.timestamp / 100)),
    );

    const componentMemoryImpact = memoryData.filter((m) =>
      renderBuckets.has(Math.floor(m.timestamp / 100)),
    );

    if (componentMemoryImpact.length > 0) {
      const avgMemoryDelta =
        componentMemoryImpact.reduce((sum, m) => sum + (m.delta || 0), 0) /
        componentMemoryImpact.length;

      if (Math.abs(avgMemoryDelta) > 5) {
        correlations.push({
  ...
  return correlations.sort(
    (a, b) => Math.abs(b.avgMemoryDelta) - Math.abs(a.avgMemoryDelta),
  );
}
